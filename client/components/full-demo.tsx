...
export function FullDemo({ initialMessage }: FullDemoProps) {
  ...
  return (
    <div className="flex h-[600px] w-full max-w-4xl" data-testid="chat-container">
      <Card className="flex grow flex-col">
          ...
          <ScrollArea className="h-full pr-4" ref={scrollAreaRef}>
            <div className="space-y-4">
              {messages.map((message) => (
                <div
                  key={message.id}
                  className={`flex items-start space-x-2 ${message.isAI ? 'justify-start' : 'justify-end'}`}
                >
                  {message.isAI && (
                    <Avatar>
                  ...
                  <div className={`flex flex-col ${message.isAI ? 'items-start' : 'items-end'}`}>
                    ...
                    <div
                      className={`mt-1 rounded-lg p-2 ${message.isAI ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
                    >
                      <p>{message.content}</p>
                    </div>
        ...
        <CardFooter className="pt-4">
          ...
          >
            ...
            <Button type="button" size="icon" variant="outline" aria-label="Attach file">
              <Paperclip className="size-4" />
            </Button>
          </form>
        </CardFooter>
      </Card>
      <Card className="ml-4 hidden w-64 flex-col md:flex">
        <CardHeader>
